...
struct indexer
{
   ...
   __device__ static inline typename std::enable_if<(M == sizeof...(Indices)), T&>::type access(
   ...
   template <class... Indices>
   __device__ static inline const std::enable_if<(M == sizeof...(Indices)), const T&> access_const(
...
struct indexer<T,0>
{
   __device__ static inline T& access( T* data, const uint32_t* ds, const short size, const size_t& ii)
   {
   ...
   __device__ static inline const T& access_const(const T* data, const uint32_t* ds, const short size,
                                       const size_t& ii)
   {
      assert(ds[0] == 1);
      size_t index = ii /** ds[0]*/;
...
struct indexer<T, 1>
{
   __device__ static inline T& access(T* data, const uint32_t* ds, const short size, const size_t& ii, const size_t& jj)
   {
      assert(ds[1] == 1);
      assert(size>=2);
   ...
   __device__ static inline const T& access_const(const T* data,
...
struct indexer<T, 2>
{
   __device__ static inline T& access(T* data, const uint32_t* ds, const short size, const size_t& ii, const size_t& jj, const size_t& kk)
   {
      assert(ds[2] == 1);
      assert(size>=3);
